...
const ReadOnlyLessonView = ({ lesson, onDownloadPDF }) => {
    const dispatch = useDispatch()
    const { categories: categoryList } = useSelector((state) => state.categories)
    const [vocabulary, setVocabulary] = React.useState(lesson.vocabulary || [])
    ...
    const handleDownloadPDF = async () => {
        if (!lesson || !lessonContentRef.current) return
        ...
        try {
            ...
            tempStyle.id = 'pdf-oklch-override-readonly'
            ...
            try {
                    ...
                    const headerSection = pageNum === 0 ? `
                        <div style="margin-bottom: 2rem; padding-bottom: 1.5rem; border-bottom: 2px solid rgb(199, 210, 254);">
                            <div style="text-align: center; margin-bottom: 1rem;">
                                ...
                                <p style="font-size: 1.25rem; color: rgb(75, 85, 99); word-break: break-word; overflow-wrap: break-word;">${lesson.titleEnglish}</p>
                            </div>
                            <div style="display: flex; justify-content: space-between; align-items: center; gap: 0.75rem; flex-wrap: wrap;">
                                ...
                                <span style="font-size: 1.25rem; color: rgb(107, 114, 128); word-break: break-word; overflow-wrap: break-word;">(${lesson.subtitleEnglish})</span>
                            </div>
                        </div>
                ...
                // Generate filename
                const filename = `${lesson.title || 'lesson'}_${lesson.subtitle || 'vocabulary'}.pdf`
                pdf.save(filename)
                ...
                const styleElement = document.getElementById('pdf-oklch-override-readonly')
                if (styleElement) {
        ...
        <div className="relative">
            ...
            <div className="absolute top-4 right-4 z-10">
                <button
                    ...
                    className="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-md transition hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                    {isGeneratingPDF ? (
                        <>
            ...
            <div className="mb-8 pb-6 border-b-2 border-indigo-200">
                <div className="text-center mb-4">
                    <h1 className="text-5xl font-bold text-gray-900 mb-2" style={{ fontSize: '3rem', lineHeight: '1.2', wordBreak: 'break-word', overflowWrap: 'break-word' }}>
                        {lesson.title}
                    </h1>
                    <p className="text-xl text-gray-600" style={{ fontSize: '1.25rem', wordBreak: 'break-word', overflowWrap: 'break-word' }}>{lesson.titleEnglish}</p>
                </div>
                <div className="flex items-center justify-between gap-3 flex-wrap">
                    <h2 className="text-4xl font-bold text-indigo-600" style={{ fontSize: '2.25rem', lineHeight: '1.2', wordBreak: 'break-word', overflowWrap: 'break-word' }}>
                        {lesson.subtitle}
                    </h2>
                    <span className="text-xl text-gray-500" style={{ fontSize: '1.25rem', wordBreak: 'break-word', overflowWrap: 'break-word' }}>({lesson.subtitleEnglish})</span>
                </div>
            </div>
...
export default ReadOnlyLessonView
